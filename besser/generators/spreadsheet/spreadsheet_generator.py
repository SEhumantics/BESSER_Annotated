...
class SpreadSheetGenerator(GeneratorInterface):
    ...
    def generate(self):
        ...
        """
        workbook = Workbook()
        self._create_class_sheets(workbook)
        self._create_association_comboboxes(workbook)
    ...
    def _create_class_sheets(self, workbook):
        """
        Creates a sheet for each class, applies styles, and fills with example values.
        """
        for buml_class in sort_by_timestamp(self.model.types):
            if not isinstance(buml_class, Class):
                continue
            sheet = workbook.create_sheet(title=buml_class.name)
                ...
                for row in range(2, self.MAX_ROWS):
                    ...
                    if row == 2:  # Set example value only in the first row for example data
                        if style == self.text_style:
                            example_value = buml_class.name + "_" + attribute.name
                        data_cell.value = example_value

    def _create_association_comboboxes(self, workbook):
        """
        Adds comboboxes to each class sheet for one-to-one associations, linked to other sheets.
        """
        for buml_class in sort_by_timestamp(self.model.types):
            for end in sort_by_timestamp(buml_class.association_ends()):
                if end.multiplicity.max == 1:
                    sheet = workbook[buml_class.name]
                    ...
                    first_option = workbook[end.type.name].cell(row=2, column=1).value
